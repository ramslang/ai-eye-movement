...
def predict():
    ...
    class_names = {0: 'eye_closed', 1: 'forward_look', 2: 'left_', 3: 'right'}
    # result = {
    ...
    return render_template("classifications.html", img_path = predict_image_path, 
                        prediction = class_names[np.argmax(prediction_array_model)],
                        confidence = '{:2.0f}%'.format(100 * np.max(prediction_array_model)),  
                        )
